...
/**
 * Modérateur Middleware
 * @module middleware/modérateur
 */
const modérateur = async (req, res, next) => {
  ...
  try {
    ...
    if (headerXsrfToken !== cookieXsrfToken) {
      console.log(chalk.red('Probléme de token csrf dans le moderteur MW'))
      return res.status(401).json({
        message: 'Probléme de token csrf'
      });
    }

    //! va falloir comprendre comment je peut pérénniser l'info du user aprés m'êre logé car ici il n'y a pas d'objet user.. et j'en ai besoin
    ...
    if (req.session.user.role !== 'Modérateur' && req.session.user.role !== 'Administrateur') {
      return res.status(403).json({
        message: 'Vous n\'avez pas les droit nécéssaires pour accéder a la ressource.'
      })
    }
    ...
    console.log(chalk.red.bold.bgYellow(`L'utilisateur ${req.session.user.pseudo} avec le role ${req.session.user.role} a bien été authentifié via le Modérateur MW !`))
